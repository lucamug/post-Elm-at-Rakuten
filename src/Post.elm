...
regex : Regex.Regex
regex =
    Maybe.withDefault Regex.never (Regex.fromString "\\{snippet_[^}]+\\}")


f : Regex.Match -> String
f match =
    case Dict.get match.match snippets of
        Nothing ->
            match.match |> Debug.log "Match not found!"

        Just snippet ->
            case target of
                DevTo ->
                    "{% gist https://gist.github.com/lucamug/" ++ snippet.gist ++ " %}" |> Debug.log "Replacing"
...
test : String
test =
    Regex.replace regex f post



-- https://gist.github.com/lucamug/eaddc29c2b268e12a322771cbe15b8e9


snippets : Dict.Dict String { code : String, gist : String }
snippets =
    Dict.fromList
    ...
    .swim !== undefined;
            ...
            , gist = "4fcd72de4fe997db8a20f4e94cb9e1cf"
            }
          )
    ...
    pet.fly();
            ...
            , gist = "eaddc29c2b268e12a322771cbe15b8e9"
            }
          )
...
a = 1;
a = a * 3;
a = a + 2; // Gives 5{{< /highlight >}}"""
            , gist = "dbb238361d9dac026cf8d7ad3ed912ed"
            }
          )
        , ( "{snippet_Example_case_of.elm}"
          , { code = """{{< highlight elm >}}case pet of
    Fish fish -> fish.swim
    Bird bird -> bird.fly{{< /highlight >}}"""
            , gist = "714a378db3d9c56df31e10338d1b3aff"
            }
          )
        , ( "{snippet_Maybe.elm}"
          , { code = """{{< highlight elm >}}type Maybe a = Just a | Nothing{{< /highlight >}}"""
            , gist = "91babe391c4e2d89d0bb41df661110bd"
            }
          )
        , ( "{snippet_declarative}"
          , { code = """{{< highlight text >}}func1 ( func2 ( func3 (...) ) ){{< /highlight >}}"""
            , gist = "62d3e76ee8c6f3d62cac7d24e942aa86"
            }
          )
        , ( "{snippet_imperative}"
    ...
    proc3();
            ...
            , gist = "53fc390b38fa5543c442264138abbd41"
            }
          )
        , ( "{snippet_Elm_function.elm}"
          , { code = """{{< highlight elm >}}add a b = a + b{{< /highlight >}}"""
...
[![John Carmack on Elm Errors]("""
        ++ imagesUrl
        ++ """john-carmack-on-errors.png)](https://twitter.com/id_aa_carmack/status/735197548034412546)
...
[^familiarity-hides-complexity]: Rich Hickey mentions the idea that *familiarity hides complexity* in his talk [Are we there yet?](https://www.infoq.com/presentations/Are-We-There-Yet-Rich-Hickey/) (11th minute) where he advocated for the reexamination of the basic principles of OOP.
